...
export default function Stats({ stats, active, type }) {
    ...
    return (
            <div className={`${active === 'stats' ? 'animate-flyIn': 'hidden' } w-80 pt-3 py-3 space-y-3 bg-gray-900 mt-7 mx-auto rounded-xl flex flex-col text-gray-100 translate-y-32 opacity-0`}>
                {stats.map((stat, index) => (
                    <div className="grid grid-cols-7 w-full gap-3 items-center" data-stat={stat.value} key={index}>
                        ...
                        <div className={`col-span-4 h-3 bg-gray-500 rounded-lg bar relative overflow-hidden after:absolute after:content-[''] after:top-0 after:left-0 after:h-full  ${stat.name === 'Sp. Atk' || stat.name === 'Sp. Def' ? afterBgColor : 'after:bg-gray-100'} after:rounded-lg`} />
                    </div>
                ))}
            </div>
    )
